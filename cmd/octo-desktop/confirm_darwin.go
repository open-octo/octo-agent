...
//
// Raising the app instead would be the obvious fix and is no longer available:
// since macOS 14 an app the user has not activated cannot activate itself, and
// activateIgnoringOtherApps: from a tray click is silently ignored ([NSApp
// isActive] stays false). Both properties below ask for none of that — they
// describe the window, not who owns the focus.
//
