...
|---|---|---|
| `--profile <name>` | — | Use an isolated user-data root at `~/.octo-<name>` instead of `~/.octo`. Global: valid in any position on any subcommand. Also `OCTO_PROFILE`. See [Run more than one octo](/docs/guides/profiles/) |
| `-c`, `--continue [id]` | — | Resume a session belonging to the current directory — `last`, a short ID, or a substring; omit the value to pick from a list |
...
| `--compact-auto-pct` | `0` (→75) | Auto-compaction threshold as % of the context window, used when `--compact-threshold=0`. The same threshold is checked both between turns and after each tool-call batch. |
| `--fallback-context-window` | `0` (→128000) | Context window in tokens to assume for a model whose name matches no built-in entry — a self-hosted or renamed model. Never overrides a model the built-in table knows; to size one known model's deployment, set `context_window` on its endpoint model entry instead. Also `OCTO_FALLBACK_CONTEXT_WINDOW` and `fallback_context_window` in the config file. |
...
| `octo sessions` [`--all`] | List this directory's sessions; `--all` lists every session on the machine, grouped by directory |
| `octo trash list`\|`restore`\|`rm`\|`empty` | Recover files the agent deleted or overwrote (the [recycle bin](/docs/guides/sandbox-the-agent/)) |
| `octo profiles list`\|`create`\|`rm`\|`path` | Manage the user-data roots `--profile` selects between ([profiles](/docs/guides/profiles/)); `rm` needs `--yes` and refuses the default, current, or running profile |
| `octo workflows list`\|`path`\|`update` | List saved workflows |
| `octo doctor` | Read-only health check of your setup |
