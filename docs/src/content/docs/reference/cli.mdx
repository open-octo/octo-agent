...
| `--fallback-context-window` | `0` (→128000) | Context window in tokens to assume for a model whose name matches no built-in entry — a self-hosted or renamed model. Never overrides a model the built-in table knows. Also `OCTO_FALLBACK_CONTEXT_WINDOW` and `fallback_context_window` in the config file. |
