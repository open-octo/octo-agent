...
| `rpm` | int | Optional: cap on requests started against this endpoint per rolling 60-second window. `0`/absent = unlimited |
| `max_concurrency` | int | Optional: cap on requests in flight against this endpoint at once. `0`/absent = unlimited |
