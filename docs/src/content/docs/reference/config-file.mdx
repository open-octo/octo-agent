...
- **TUI / CLI** (`octo`, `octo -c`) always run tools in the directory you launched them from —
  ...
  against.
- **Web sessions** get theirs in two steps: a new session with no directory of its own defaults to
  `workspace_dir` (empty → `~/Octo`, created on first use); the directory chip in the Composer (or
  `PATCH /api/sessions/{id}/working_dir`) then overrides that default for just that session, and
  the override sticks even if `workspace_dir` changes later.
