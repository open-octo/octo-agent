...
|---|---|---|
| `explore` | yes | Research on the parent's model, with a trimmed system prompt |
| `plan` | yes | Read-only investigation that produces an implementation plan; runs on the cheaper lite model when one's configured |
...
injection from their system prompt, since a quick research pass rarely needs either. `plan`
additionally runs on the lite model; `explore` deliberately keeps the parent's model, because
its findings gate the parent's next step and a downgraded scout returns downgraded findings.
