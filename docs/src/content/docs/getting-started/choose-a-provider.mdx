...
- **No API key is needed.** Leave `api_key` out (or blank) and octo sends no `Authorization`
  header. The `custom` vendor is the only one that may run keyless; a named vendor such as
  `openai` still insists on a key, so point local servers at `custom`.
...
`CLI flag > env var > ~/.octo/config.yml`. The **model** falls back to a per-vendor default; the
**vendor itself has none** — name it with `--provider`, `OCTO_PROVIDER`, or `octo config`. A key in
the environment doesn't name one: it says which vendors you can reach, not which you meant, so an
install that never names a vendor counts as unconfigured. API keys are read from
