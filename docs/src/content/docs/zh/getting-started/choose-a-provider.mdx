...
**命令行 flag > 环境变量 > `~/.octo/config.yml`**。**模型**有各家的内置默认值，**用哪家没有默认值**——
必须用 `--provider`、`OCTO_PROVIDER` 或 `octo config` 指明。环境变量里的 key 不算指明：它只说明你能连到哪些家，
不代表你想用哪家，所以从没指明过用哪家的安装会被当作未配置。API key 优先从 `ANTHROPIC_API_KEY` /
`OPENAI_API_KEY` 读取；向导可以把 key 存进文件（权限 `0600`），但更推荐用环境变量。
</Aside>
