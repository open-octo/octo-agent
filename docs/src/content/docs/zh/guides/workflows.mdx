...
| `skill(name, params = {}, opts = {})` | Hash / Array / 标量 | 把一个具名 `SKILL.md` skill 跑成子代理。和 `agent` 不同，带 `schema` 的结果会直接解析好返回 |
| `recording(name, params = {})` | Hash | 确定性地回放一段具名浏览器录制，返回它声明的 outputs |
...
下一步：浏览器录制可以通过 `recording("<name>")` 直接接进工作流脚本——见
