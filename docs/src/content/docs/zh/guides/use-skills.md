...
octo 开箱自带 18 个 skill。每一个都会在模型判断任务匹配时自动触发——大多数情况下你不需要按名字调用它们。
...
| `find-skills` | 帮你找到能完成某件事的已装 skill |
