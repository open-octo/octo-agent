...
octo 开箱自带 22 个 skill。每一个都会在模型判断任务匹配时自动触发——大多数情况下你不需要按名字调用它们。
...
| `artifact-design` | Artifacts 面板里任何自包含 HTML/Markdown 页面的设计指引——报告、仪表盘、架构图、生成式 UI |
| `dataviz` | 图表/仪表盘的绘制规则——图型选择、分类/连续/发散配色系统、图例/坐标轴/tooltip 约定 |
...
| `deep-research` | 深度多来源调研——扇出搜索、阅读原始资料、对抗性验证结论，并合成带引用的研究报告 |
