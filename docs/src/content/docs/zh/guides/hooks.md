...
注入的内容是给模型看的上下文，不是对话的一部分：octo 会先把它包进 `<system-reminder>` 再折叠进这一轮，
而渲染对话记录的界面（Web UI、TUI）会隐藏这类片段。一个每轮往提示词前面塞十行历史记忆的检索 hook，
对用户来说是不可见的——用户只会看到自己敲的那句话。模型会把这段内容当作运行时给出的指引来读，
所以不要把不可信文本（网页内容、第三方笔记）原样透传，要像内置的记忆召回那样标明它只是参考资料。
