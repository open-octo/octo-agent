...
从空白记录开始——各次运行之间从不共享会话。每个任务还会自动拥有一个以任务名命名的运行聚合，
侧边栏的**「定时任务」**区就是它——每个定时任务一行，装着它的全部运行。这一行属于调度器不属于你：
不能改名、不能单独删除、也不能把别的会话塞进去。它随任务出现、随任务改名，任务删除时一并消失
（运行产生的会话本身保留在磁盘上，变成普通任务）。
...
`directory`、`notify`——只发你要改的那部分就行；用 `name` 改名也会同步重命名任务在侧边栏「定时任务」区里的那一行。Web UI 里的调度器面板就是这套 API 的一个
