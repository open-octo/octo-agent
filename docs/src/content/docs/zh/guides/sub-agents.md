...
| 类型 | 只读 | 说明 |
|---|---|---|
| `explore` | 是 | 调研跑在父模型上，只精简系统提示 |
| `plan` | 是 | 只读式调查，产出一份实施计划；配置了更便宜的 lite model 时会跑在那上面 |
| `general` | 否 | 完整工具集，用于端到端委派 |
| `code-review` | 是 | 通过 `git diff` 做 review |

`explore` 和 `plan` 是仅有的两个"lean"类型——它们的系统提示里会去掉 skills 清单和记忆注入，
毕竟一次快速调研通常两者都用不上。`plan` 额外跑在 lite model 上；`explore` 刻意保留父模型——
它的调研结论直接决定父代理的下一步，侦察兵降级，情报也跟着降级。
