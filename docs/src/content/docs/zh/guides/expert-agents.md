...
除了你自己创建的专家，**Agents** 面板还内置了一批官方策划、面向日常场景（非编码）
的专家人设——文案助手、简历教练、旅行规划师等。可以搜索或按分类筛选，点击卡片
查看简介、擅长领域标签，以及几条可以直接点击发送的示例提问。**召唤**按钮会
开启一个绑定该专家的新会话。

内置专家的行为和普通专家智能体一样（独立 system prompt、工具白名单、独立会话池），
只有几点不同：

- **只能隐藏，不能删除。** 可以通过卡片上的隐藏操作（或直接让 Default Agent
  帮你操作）把某个内置专家从画廊里隐藏——隐藏后它不再出现在画廊里、也不能被
  选为新会话的专家，但不会丢失任何东西，再次切换即可重新显示。
- **只读。** 内置专家的 system prompt 和设置不能修改，每台机器上都一样，并持续
  接收 octo 对该人设的内容更新。想要不同的版本，就让 Default Agent 以它为蓝本
  新建一个你自己的专家，需要的话再把原版隐藏。
- **不能删除。** 只有你自己创建的专家才能被永久删除；内置专家只能隐藏。

## 创建智能体

### Web UI

1. 打开 **专家** 面板（侧边栏 → 专家，或按 `Cmd+K` → "专家"）。
2. 点击 **创建专家**。会打开一个新会话，Default Agent 通过 `expert-agent-manager`
   技能和你一起设计这个专家：先问清楚它唯一的职责、绝对不能做什么，再起草
   system prompt 给你确认，最后补上模型、工具、技能，确认后创建。
...
在专家卡片上点击 **通过对话编辑**。新会话会直接给出一张预填了当前值的编辑表单：
名称、描述、system prompt、模型、工具、技能。

- 直接改想改的字段，点保存，改动立即生效。
- 或者在表单最下面的输入框里用一句话描述想怎么改（例如"语气再正式一点"），
  由 Default Agent 替你调整；涉及 system prompt 的改动会先给你确认。

内容过长的字段（例如很长的 system prompt）不会放进表单，以免被截断，这类改动
通过最下面的输入框描述即可。继承全部工具的专家同理，表单里没有工具一栏。
