...
层级的东西必须走大版本，并且一定在[发布说明](https://github.com/open-octo/octo-agent/releases)
里带上 **Breaking** 标注。
...
| `~/.octo/mcp.json` | `mcpServers` 的结构（stdio 的 `command`/`args`/`env`，HTTP 的 `url`/`headers`/`auth`） |
| `~/.octo/channels.yml` | 按平台名分组的 `channels` map，每个平台文档化的字段 |
| `SKILL.md` | YAML frontmatter + markdown 正文，Claude Code 的格式，从 `~/.octo/skills/` 发现 |
...
文档化且真实存在，但变更会随小版本发布并在发布说明里说明，而不需要走大版本：
...
`.bak` 后缀。一个被弃用的格式或 flag 拼写，在发布说明宣布之后至少还会在一个小版本里继续可用；
彻底移除对旧格式的读取支持是破坏性变更，只会发生在大版本里。

一个 Stable 层级在没有走大版本、或者发布说明里没有 **Breaking** 标注的情况下被破坏，那就是一个
