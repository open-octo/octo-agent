...
| `/model` | `/model <name>` | 原地切换模型——同一个 provider，为这个模型刷新工具集 |
...
:::caution[IM 里不支持 skill 触发]
`/<skill-name>` 在 IM 渠道里**不生效**。任何以 `/` 开头的文本都只会去匹配上面这张固定表格；
匹配不上就回"Unknown command"，不会落到某个 skill 或者模型身上。想用某个 skill 就直接用大白话说，
模型会自己判断加载哪个。
:::
