...
在 TUI 里，`Ctrl+B` 会把当前正在跑的同步命令转成一个可见、可追踪的后台进程；Web UI 有个按钮效果
一样。IM 里没有转正的入口——只有超时会生效。

在**子代理**内部，后台能力被整个关掉：每一条 `terminal` 调用都同步执行（传了 `run_in_background`
或 `detached` 也会被忽略），命中 120 秒超时的命令会被直接杀掉并返回错误，而不是转正（所以 `Ctrl+B`
/ 那个按钮也够不着它）。子代理在启动它的那一个回合内就要返回，之后没有回合来收割后台进程的输出——
让它起后台进程还会把完成通知泄漏进父会话、且无从归属。真正长时间运行的命令应该放在父代理里跑，
而不是子代理。
