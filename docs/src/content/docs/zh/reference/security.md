...
| agent 用破坏性命令 wipe 掉整个系统 | 权限引擎硬编码 `deny`：`rm -rf /`、`dd`、`mkfs`、`fdisk`、`format`、`shutdown`、`reboot`、`kill -9 -1`，系统目录删除（`rm -rf /usr`，以及 `rmdir /s /q C:\Windows` 等所有指向 `C:\Windows` / `C:\ProgramData` / `C:\Program Files` 的删除写法，含 PowerShell `ri`/`del`/`erase` 别名），外加 `diskpart`、`bcdedit`、`reg delete`、`vssadmin delete`、`wbadmin delete`；这些规则无法被 `~/.octo/permissions.yml` 覆盖 |
...
| agent 通过 Windows 管理工具破坏系统配置 | 默认 `ask`：`wmic`、`sc`、`schtasks`、`netsh`、`icacls`、`takeown`、`robocopy`、`xcopy`、`mklink`、`dism`、`fsutil`、`powershell`、`cmd /c`，以及不带系统目录的递归删除（`rmdir /s /q`、`del /s /q`——cmd.exe 版的 `rm -rf`） |
| 事后追查 | 只追加的 JSON 审计日志 `~/.octo/audit.log`，记录每一次 deny、ask-denied、user-declined 和 user-allowed 决策 |
...
命令类规则锚定在命令词本身——经过 `sudo`/`env`/`xargs` 包装、shell 链式调用（`;`、`&&`）或
`/sbin/reboot` 这样的路径调用仍会命中，但参数里的普通文本**不会**误触发：
`git commit -m "fix shutdown handling"`、`docker ps --format json` 都不受影响。
...
和 `user-allowed`（用户点击确认放行）。记录的 input 中字符串值截断到 1 KiB——审计日志回答的是
"什么被拒了、为什么"，不归档文件内容或命令载荷。日志文件权限为 `0600`；启动时超过 10 MiB 会
轮转一次（保留 3 代），运行中永不截断。
