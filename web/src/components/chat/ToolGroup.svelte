...
{:else if tools === null}
  ...
  <div class="tool-group">
    <div class="group-header">
      <iconify-icon icon="ant-design:tool-outlined" width="14" style="color:rgba(0,0,0,0.45)"></iconify-icon>
      <span class="hdr-label">9 tools used</span>
      <span class="err-badge">
        <iconify-icon icon="ant-design:close-circle-outlined" width="12"></iconify-icon>
        1 failed
      </span>
      <span class="hdr-time mono">2.4s</span>
    </div>

    <details open class="tool-item">
      <summary class="tool-summary">
        <iconify-icon icon="lucide:chevron-down" width="13" style="color:rgba(0,0,0,0.35)"></iconify-icon>
        <iconify-icon icon="ant-design:search-outlined" width="14" style="color:rgba(0,0,0,0.45)"></iconify-icon>
        <span class="tool-name mono">grep</span>
        <span class="tool-arg mono">"SetReadDeadline" internal/ws</span>
        <span style="margin-left:auto" class="tool-meta">12 matches</span>
        <iconify-icon icon="ant-design:check-circle-outlined" width="14" style="color:#52C41A;flex:0 0 auto"></iconify-icon>
      </summary>
      <pre class="tool-output">{'internal/ws/client.go:142:  conn.SetReadDeadline(0)\ninternal/ws/client.go:188:  c.conn.SetReadDeadline(time.Time{})\ninternal/ws/pool.go:74:   ws.SetReadDeadline(deadline)'}</pre>
    </details>

    <details open class="tool-item">
      <summary class="tool-summary">
        <iconify-icon icon="lucide:chevron-down" width="13" style="color:rgba(0,0,0,0.35)"></iconify-icon>
        <iconify-icon icon="ant-design:edit-outlined" width="14" style="color:rgba(0,0,0,0.45)"></iconify-icon>
        <span class="tool-name mono">edit_file</span>
        <span class="tool-arg mono">internal/ws/client.go</span>
        <span style="margin-left:auto" class="tool-meta">2 changes</span>
        <iconify-icon icon="ant-design:check-circle-outlined" width="14" style="color:#52C41A;flex:0 0 auto"></iconify-icon>
      </summary>
      <div class="diff-block">
        <div class="diff-hdr mono">@@ internal/ws/client.go:142 @@</div>
        <div class="diff-line rm mono">- conn.SetReadDeadline(0)</div>
        <div class="diff-line rm mono">- time.Sleep(time.Second)</div>
        <div class="diff-line add mono">+ conn.SetReadDeadline(time.Now().Add(pongWait))</div>
        <div class="diff-line add mono">+ backoff := newExpBackoff(time.Second, 30*time.Second)</div>
        <div class="diff-line add mono">+ time.Sleep(backoff.Next())</div>
      </div>
    </details>
    ...
    <details open class="tool-item">
      <summary class="tool-summary">
        <iconify-icon icon="lucide:chevron-down" width="13" style="color:rgba(0,0,0,0.35)"></iconify-icon>
        <iconify-icon icon="ant-design:code-outlined" width="14" style="color:rgba(0,0,0,0.45)"></iconify-icon>
        <span class="tool-name mono">bash</span>
        <span class="tool-arg mono">go test ./internal/ws/...</span>
        <span style="margin-left:auto;display:flex;align-items:center;gap:4px;font-size:12px;color:#1677FF;flex:0 0 auto">
          <iconify-icon icon="ant-design:loading-outlined" width="13" style="animation:octo-spin 0.8s linear infinite"></iconify-icon>
          running
        </span>
      </summary>
      <pre class="terminal-output"><span style="color:#52C41A">$</span> go test ./internal/ws/...
<span style="color:rgba(255,255,255,0.55)">=== RUN   TestExpBackoff_Capped</span><span class="blink-caret"></span></pre>
    </details>

    <details open class="tool-item">
      <summary class="tool-summary">
        <iconify-icon icon="lucide:chevron-down" width="13" style="color:rgba(0,0,0,0.35)"></iconify-icon>
        <iconify-icon icon="ant-design:file-text-outlined" width="14" style="color:rgba(0,0,0,0.45)"></iconify-icon>
        <span class="tool-name mono">read_file</span>
        <span class="tool-arg mono">internal/ws/legacy.go</span>
        <span style="margin-left:auto;display:flex;align-items:center;gap:4px;font-size:12px;color:#FF4D4F;flex:0 0 auto">
          <iconify-icon icon="ant-design:close-circle-outlined" width="14"></iconify-icon>
          not found
        </span>
      </summary>
      <div class="error-output mono">Error: open internal/ws/legacy.go: no such file or directory</div>
    </details>
  </div>
{/if}
