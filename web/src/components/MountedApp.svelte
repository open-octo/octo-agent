<script lang="ts">
  ...
  // rendered in the same frame the Light Apps panel uses (ArtifactsPanel),
  // under the same sandbox. It also covers the case of a slug the list no
  // longer has, which is what a deleted app leaves behind in a bookmarked hash.
  ...
  // rebuild the src — the same thing the artifact frame does. The src waits
  // for the app's old storage to be moved into its namespace (laStorage.ts).
  $effect(() => {
    void ensureLightAppStorage(slug)
  })
  const src = $derived.by(() => {
    void $themeRev
    return $lightappStorageReady.has(slug) ? lightappURL(slug) : ''
  })
  ...
  // silently loses the desktop download path.
...
{:else if $lightapps.length === 0 || app}
