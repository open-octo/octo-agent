...
// least as many times, indented no more than three spaces (four makes it
// content). marked hands an unclosed trailing fence to the code renderer too,
...
function isClosedFence(raw: string): boolean {
  ...
  const close = /^ {0,3}(`{3,}|~{3,})$/.exec(body.slice(nl + 1))
...
renderer.code = function ({ text: codeText, lang, raw }: Tokens.Code) {
  ...
  // lang is the whole info string: "mermaid title" or "Mermaid" still count.
  if (lang?.split(/\s/)[0].toLowerCase() === "mermaid" && isClosedFence(raw)) {
