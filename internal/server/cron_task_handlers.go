...
func (s *Server) handleRunCronTask(w http.ResponseWriter, r *http.Request) {
	...
	// The session is created synchronously so the UI can open it immediately.
	for _, t := range s.scheduler.List() {
		if t.Name == name || t.ID == name {
			sessionID, err := s.scheduler.RunNow(t.ID)
			if err != nil {
				writeError(w, http.StatusBadRequest, err.Error())
				return
			}
			writeJSON(w, http.StatusAccepted, map[string]string{"status": "started", "id": t.ID, "session_id": sessionID})
			return
		}
	}
