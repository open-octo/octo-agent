...
按脚本输出处理：
- `exit 0` → 继续
- `exit 2` → 需询问用户偏好，写入 `<SKILL_DIR>/config.env` 的 `WEB_ACCESS_BROWSER`
...
| URL 已知，需要从页面定向提取特定信息 | **web_fetch**（拉取网页内容，由模型根据 prompt 提取，返回处理后结果） |
| URL 已知，需要原始 HTML 源码（meta、JSON-LD 等结构化字段） | **terminal**（执行 curl） |
...
- **看**：用 `/eval` 端点查询 DOM，发现页面上的链接、按钮、表单、文本内容——相当于「看看这个页面有什么」
- **做**：用 `/click` 端点点击元素、`/scroll` 端点滚动加载、`/eval` 端点填表提交——像人一样在页面内自然导航
- **读**：用 `/eval` 端点提取文字内容，判断图片/视频是否承载核心信息——是则提取媒体 URL 定向读取或 `/screenshot` 端点截图视觉识别
...
**⚠️ 重要：不要直接用 `terminal` 工具启动 `cdp-proxy.mjs`**。`check-deps.mjs` 内部通过 `spawn` + `detached:true` + `unref()` 启动 Proxy，使其完全脱离当前终端进程。如果你直接通过 `terminal` 以 background 模式启动 proxy，它会被 octo 的 TUI 持续追踪（一直转圈），且 proxy 随 terminal session 结束而终止。

每次联网任务前，只需运行 `check-deps.mjs`：若 proxy 已在运行则秒过；若未运行则自动以 detached 模式拉起后返回。`check-deps.mjs` 返回 exit 0 后，proxy 就已经独立运行在后台，后续所有 curl 调用 `localhost:3456` 即可。
...
判断内容在图片里时，用 `/eval` 端点从 DOM 直接拿图片 URL，再定向读取——比全页截图精准得多。
...
- `/scroll` 端点到底部会触发懒加载，使未进入视口的图片完成加载。提取图片 URL 前若未滚动，部分图片可能尚未加载。
...
用户浏览器真实渲染，截图可捕获当前视频帧。核心能力：通过 `/eval` 端点操控 `<video>` 元素（获取时长、seek 到任意时间点、播放/暂停/全屏），配合 `/screenshot` 端点采帧，可对视频内容进行离散采样分析。
...
用 `/close` 端点关闭自己创建的 tab，必须保留用户原有的 tab 不受影响。
