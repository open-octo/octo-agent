...
AI Agent 原本的联网能力（web_search、web_fetch）缺少调度策略和浏览器自动化能力。这个 Agent Skill 补上的是：**联网策略 + CDP 浏览器操作 + 站点经验积累**。兼容所有支持 SKILL.md 的 Agent（octo、Cursor、Gemini CLI、Codex CLI 等）。
...
| 能力 | 说明 |
|------|------|
| 联网工具自动选择 | web_search / web_fetch / terminal（curl）/ Jina / CDP，按场景自主判断，可任意组合 |
...
**v2.5.3 更新：**
- **`/new` 和 `/navigate` 改 POST body** — 避免目标 URL 含 query 时被错误切分。旧写法返回 400 + 迁移指引。详见 `references/migration-2.5.3.md`
...
- **修复 SKILL_DIR 路径问题** — bash 代码块改用绝对路径，修复 Windows Git Bash 路径转换错误和变量未设置问题（#47 #46）
...
```bash
git clone https://github.com/eze-is/web-access ~/.octo/skills/web-access
```
...
skill 长期偏好保存在 `<SKILL_DIR>/config.env`（首次运行自动从 `config.env.template` 创建，gitignored）：

```bash
# 留空 = 每次启动都问用户偏好；设值 = 固定使用该浏览器；若未检测到对应 toggle 则硬错（不擅自降级）
#
# 合法值：chrome / edge
WEB_ACCESS_BROWSER=
```
...
```bash
pkill -f cdp-proxy.mjs && node "<SKILL_DIR>/scripts/check-deps.mjs"
```
...
# 手动运行请替换为实际路径，如 ~/.octo/skills/web-access
```
