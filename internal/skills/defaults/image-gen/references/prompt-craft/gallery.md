...
Each `gallery-*.md` category file contains the concrete prompts, image paths, metadata, and attribution/source markers for that slice of the Gallery Atlas.

> **Note:** the `docs/*.png` preview images referenced by the category files are **not bundled** in this skill (they are large; see PROVENANCE.md). The entries remain fully useful as **prompt text + metadata** — read the `**Prompt**` blocks; the missing previews don't affect prompt authoring. Browse the full previews at <https://github.com/wuyoscar/GPT-Image2-Skill>.
