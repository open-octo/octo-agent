...
| Key | Type | Description |
|---|---|---|
| `rpm` | int | Most provider calls octo starts against this endpoint per rolling 60-second window. `0`/absent = unlimited |
| `max_concurrency` | int | Most provider calls in flight at once against this endpoint. `0`/absent = unlimited |
...
- Transient 429s are retried with backoff regardless of these settings (up to 4 attempts, honouring
  ...
  than breaking. **Retries don't spend a second `rpm` permit** — one call that keeps failing can put
  up to 4 requests on the wire against one permit, so set `rpm` a little under the published quota
  if the endpoint counts rejected requests too.
- **`max_concurrency` slows background work down as well.** Session titles are generated by a
  throwaway model call with a 5-second budget, fired when your message arrives — so with
  `max_concurrency: 1` it queues behind the turn's own call, usually runs out of time, and the
  session falls back to a snippet of your first message. That fallback is by design; if you'd
  rather keep model-written titles, leave `max_concurrency` unset and cap `rpm` alone.
