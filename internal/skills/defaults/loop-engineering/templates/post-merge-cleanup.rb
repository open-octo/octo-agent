...
STATE_REL = ".octo/post-merge-cleanup-state.md"
...
discover_prompt = [
  "You are cleaning up the git repository at #{repo} after merges.",
...
if safe.empty? && unsafe.empty?
  write_report(repo, STATE_REL, "# Post-Merge Cleanup State\n\n**Action:** nothing to clean.\n")
  "post-merge-cleanup: no merged branches to clean up."
else
  # ---- plan ticket updates -------------------------------------------------------
  ...
  if apply && !safe.empty?
    delete_prompt = [
      "Delete these local git branches in the repository at #{repo}, using `git branch -D <name>`:",
      safe.map { |b| b["name"] }.join(", "),
      "Never delete a branch matching any of these protected patterns: #{protect.join(', ')}.",
      "If a branch name matches a protected pattern, skip it and list it under skipped instead.",
      "Return JSON matching #{DELETE_SCHEMA}.",
    ].join("\n")
    raw_delete = agent(delete_prompt, { "read_only" => false, "schema" => DELETE_SCHEMA })
    deleted = (JSON.parse(raw_delete) rescue { "deleted" => [] })["deleted"] || []
  ...
  # ---- write state --------------------------------------------------------------
  state_lines = ["# Post-Merge Cleanup State", "", "**Mode:** #{apply ? 'apply' : 'dry run'}", ""]
