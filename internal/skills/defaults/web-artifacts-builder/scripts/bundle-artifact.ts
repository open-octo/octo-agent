...
// PostCSS plugins — it has no idea what `@tailwind base/components/utilities`
// means and would ship those directives through to the browser verbatim
// (i.e. the artifact would render completely unstyled). Precompile the real
// utility CSS with the `tailwindcss` CLI (already a project devDependency)
// and swap it in for the build, restoring the original source afterward so
// the user's editable src/index.css is left untouched on disk.
...
// scaffold's own index.html (the favicon link). Build from a temporary copy
// of index.html with such references rewritten to point at "public/", so we
// never need to mutate the user's actual index.html on disk.
let entryHtml = fs.readFileSync(indexHtmlPath, "utf8");
entryHtml = entryHtml.replace(/((?:href|src)=")\/([^"]+)(")/g, (full, prefix, assetPath, suffix) => {
