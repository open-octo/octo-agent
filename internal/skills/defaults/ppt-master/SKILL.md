...
| `${SKILL_DIR}/scripts/analyze_images.py` | Image fact analysis (measured width/height/aspect for the deck's `images/` folder) |
| `${SKILL_DIR}/scripts/rotate_images.py` | Source-intake image orientation normalization |
| `${SKILL_DIR}/scripts/latex_render.py` | LaTeX formula rendering (manifest-driven PNG assets) |
| **image-gen skill** | AI image generation, web image search, sheet slicing — `image_gen.py` / `image_search.py` / `slice_images.py` live in the separate `image-gen` skill (see Step 5's delegation note). |
...
> [!IMPORTANT]
> ## 🎨 Image acquisition is delegated to the `image-gen` skill
>
> Generating, searching, and slicing images is **not** implemented in this skill — it lives in the separate general-purpose **`image-gen`** skill. **Load the `image-gen` skill now** and perform the acquisition through it.
>
> **Everywhere below** (and throughout `references/`, `workflows/`, and `scripts/docs/`) that this skill names `references/image-base.md` / `references/image-generator.md` / `references/image-searcher.md`, the `references/image-palettes|image-renderings|image-type-templates/` catalogs, or the `image_gen.py` / `image_search.py` / `slice_images.py` commands — **those files and scripts now live in the `image-gen` skill**, not here. Read/run them from that skill's directory. (`analyze_images.py`, `rotate_images.py`, and `latex_render.py` stay in ppt-master — they operate on the deck's own image facts and source-intake images, not the acquisition path.)
>
> **Handoff contract**: ppt-master owns the resource list (`design_spec.md §VIII`) and writes the manifest `<project_path>/images/image_prompts.json`; the `image-gen` skill reads it, runs each row's path, and writes each item's status (`Generated` / `Sourced` / `Failed` / `Needs-Manual`) back into the manifest + `image_sources.json`. Formula rows (`Acquire Via: formula`) stay in ppt-master's LaTeX path and are out of scope for `image-gen`.

**Always load the common framework** (from the `image-gen` skill):

```
Read the image-gen skill's references/image-base.md
```

Then **lazy-load the path-specific reference** for each row that actually needs it (all paths below are inside the `image-gen` skill):
