...
[Website](https://octo-agent.dev) · [简体中文](README.md) · [Install Guide](https://octo-agent.dev/docs/getting-started/install/) · [Documentation](https://octo-agent.dev/docs/) · [Community](#community)
...
- **Everywhere you are**: the same binary serves eight entry points — CLI, web, desktop, IM, editor extensions, SDK, and mobile.
...
**Stable (1.0)** already covers CLI, Web UI, Desktop, IM bridge, [VS Code](https://github.com/open-octo/octo-vscode) / [Obsidian](https://github.com/open-octo/octo-obsidian) extensions, and the [Go SDK](pkg/octoagent). The eighth — a mobile app (iOS + Android) — is implemented and in **developer preview**: buildable from source today against a self-hosted relay (see [`mobile/`](mobile/)), with a hosted relay and app-store builds next.
...
┌────────────────────────────────┐
│        Eight interfaces        │
│  CLI/TUI · Web · Desktop · IM  │
│ VS Code · Obsidian · SDK · App │
└───────────────┬────────────────┘
                │
┌───────────────▼────────────────┐
│         App bootstrap          │
...
octo has shipped **1.0 stable** — CLI, Web UI, desktop, IM bridge, editor extensions, and the Go SDK are all safe to rely on; mobile is in developer preview. What you can build on is declared in [COMPATIBILITY.md](COMPATIBILITY.md); the security boundary in [SECURITY.md](SECURITY.md).
