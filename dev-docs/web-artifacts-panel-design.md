...
- **HTML**: rendered as a page of its own. The panel asks `POST /api/sessions/{id}/artifacts/grant` (same transcript whitelist as the content endpoint, html only) and loads `/_artifacts/<token>/` by `src` into `<iframe sandbox="allow-scripts allow-same-origin allow-forms allow-modals allow-downloads allow-pointer-lock">`. The page is on the app's own origin and behind the same auth as `/api`, so it works however octo is reached — this machine, the desktop app, a tunnel, a domain. It resolves relative references (`./app.js`, `./chart.png`, `./model.glb`) against its own directory, where any file is served, and may load any external resource. The iframe keeps its styles and globals out of the UI, and a script the server splices in ahead of the page's own (`internal/server/page_shim.js`) keeps its `localStorage` keys under a per-page prefix so it cannot clear the UI's settings. The page is not isolated from the app beyond that — same-origin script can call the API as the user, an accepted trade. The sandbox stays for `allow-popups` and `allow-top-navigation`, which keep a page from opening windows or navigating the host tab away. The mobile UI has no artifact preview. Full account: `same-origin-artifacts-design.md`.
