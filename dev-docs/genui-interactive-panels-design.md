...
**Panel pending state.** Sending a panel action marks that panel pending; `GenuiBlock` renders a subdued busy state plus a status chip pinned to the panel's top-right (a spinning "updating panel…"), and `dispatchAction` refuses further actions while pending. Local interaction stays live throughout — only actions are refused — so the panel must still look usable. The chip is an overlay with `pointer-events: none`, so it never shifts layout or blocks a control.

**The view stays at the panel.** A silent action skips `send()`'s usual pin-to-bottom (`pin=false`): the reply updates the panel in place, so the user acting on a panel from up in the scrollback keeps their position. When the turn completes with a genuinely new panel version, the chip flips to a transient "panel updated · 5s · 1.6k tokens" confirmation (the turn's cost, handed over by the turn-done handler via `panelTurnStats`) and fades out after a few seconds. The confirmation is gated on the spec's content actually changing across the pending window, not on the pending flag alone — a turn that ends without updating the panel (error, interrupt, reply degraded to a visible bubble) must not claim success. If the reply degrades to a visible bubble, the turn-done handler re-pins the view to the bottom so the model's answer isn't stranded off-screen.
...
**Transcript honesty.** A hidden pair is hidden, not deleted — it is in history, in the session file, and in any export, so the record always explains why a panel holds the state it does. The live transcript, however, draws nothing at all for a silent pair: acting on a panel is manipulation of an object, not a thing said, and a marker row at the tail of the message flow read as a stray orphan line. In-conversation feedback lives entirely on the panel's status chip; anyone needing the raw action JSON reads it from the export or session file.
...
| 5 | Silent classification, happy path | Action with `panel:"p"`, reply is exactly one fence with `id:"p"` | Both messages hidden, nothing drawn in the transcript; panel content swaps; panel chip flashes "updated" with the turn's stats |
| 6 | Degrade: prose in reply | Same action, reply has a sentence before the fence | Reply renders as an ordinary bubble; panel leaves pending without claiming "updated"; view re-pins to bottom so the reply is seen |
