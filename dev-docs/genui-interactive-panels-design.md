...
1. Its immediately preceding message in the array is a hidden action message.
...
**Streaming.** The classification above needs the finished message, but the user must not stare at nothing while it streams. The reply is therefore treated as *provisionally silent* and re-evaluated on every delta by a cheap monotone predicate: content trimmed is empty, or it starts with ` ```octo-ui ` and has not yet produced any non-whitespace text after a closing fence. The moment that predicate goes false the message begins rendering as an ordinary bubble mid-stream and stays that way — the predicate never flips back. In the common case (the model does emit exactly one fence) the reply never appears as a bubble at all, and the panel swaps to the new spec when the turn ends.

**Panel pending state.** Sending a panel action marks that panel pending; `GenuiBlock` renders a subdued busy state and `dispatchAction` refuses further actions while pending (the same early return `GenuiBlock.svelte:39` already uses for a non-interactive tree). Pending clears when the turn ends, when the reply is reclassified as an ordinary bubble, or on turn error/interrupt — in the last case the panel keeps its current spec and shows an inline error line. Pending is in-memory only: a reload mid-turn drops it, and the projection re-derives the correct panel content from whatever did land in history.
...
`GenuiMermaid.svelte` imports mermaid dynamically at first render — `const { default: mermaid } = await import('mermaid')` — mirroring the html2canvas precedent at `ChatView.svelte:1962` and its stated reason. It initializes with `securityLevel: 'strict'` (`config.type.d.ts:163` lists `'strict' | 'loose' | 'antiscript' | 'sandbox'`), renders to an SVG string, and shows a compact error line if mermaid rejects the source — a model writing invalid diagram syntax must never blank the panel.
...
- **Reserved field names.** Model-supplied `field` values may not begin with `__`, keeping internal state (tab indices) unaddressable from a spec.
