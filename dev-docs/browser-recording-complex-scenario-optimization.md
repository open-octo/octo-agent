...
**方案**：录制期间把 auto-attach 切到 `waitForDebuggerOnStart: true`——新开的 Tab 在**创建那一刻就被冻结**（不加载、不渲染、不可点击），直到 attach watcher 完成 instrument（binding + on-new-document 脚本 + 对当前文档 evaluate）后调用 `Runtime.runIfWaitingForDebugger` 恢复。captureScript 保证先于页面第一个文档 commit 就位，竞态在协议层面不可能发生。`Stop()` 恢复非冻结模式。

```go
// Start()：录制期间冻结新目标
Target.setAutoAttach{autoAttach:true, waitForDebuggerOnStart:true, flatten:true}
// attach watcher：instrument 完成后无条件恢复（冻结的 Tab 比未插桩的 Tab 更糟）
Runtime.runIfWaitingForDebugger
// Stop()：恢复 waitForDebuggerOnStart:false
```

**决策依据**：相比"加 wait 步骤回放时再等"，在源头修复更彻底——不依赖回放时的等待策略。冻结-恢复是 CDP 为这类竞态设计的原生机制。

**踩坑记录（两版失败方案）**：
1. 初版 `Page.stopLoading()` + `Page.navigate` reload：`Runtime.evaluate` 的 envelope 被直接 `json.Unmarshal` 进 `string` 永远失败，`curURL` 恒空、reload 实为死代码；本地全绿是原始导航自然完成掩盖的。CI 慢机器上 stopLoading 真的取消了首次导航 → Tab 永远困在 about:blank。
2. 二版修好解析 + stranded 恢复（`Target.getTargetInfo` / attach 事件的 `targetInfo.url` + 轮询）：CI 上导航被取消得太早，目标 URL **任何地方都没记录**（`target=_blank` 的 Tab 以 about:blank 创建，导航被取消后 targetInfo 永远不更新），恢复无目的地。同时实测发现对已 commit 页面 reload 会稳定丢失紧随其后的点击。
结论：stopLoading 路线在"未 commit 的 Tab"上根本不可救；freeze-on-create 一次解决全部时序问题。
...
5. ~~新 Tab reload 对一次性 URL 的影响~~：已随 freeze-on-create 方案消除——录制不再对任何 Tab 做 re-navigate，一次性 URL（OAuth 回调、支付跳转）不受影响
