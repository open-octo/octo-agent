...
5. **保存 + 指路** —— `workflow_save`(`name`),恒写入 `~/.octo/workflows`(跨项目可用)。存好后告诉用户三种跑法:会话点名调、CLI 点名、或用 `cron-task-creator` 配定时。（曾经有 `scope: "project"|"user"` 二选一,砍掉项目级后不再需要。）
...
- **不自动保存** —— 保存是显式一步(用户确认 `name`),绝不偷偷写盘。
