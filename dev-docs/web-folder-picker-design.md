...
The web backend already runs the agent on the local filesystem with the same file tools as the CLI. A session runs its tools in the directory of the project it belongs to, and choosing a directory is how a new session gets filed under one (see projects-design.md — a session cannot carry a directory of its own). What is missing is a way for the user to *choose* that path from the browser: a browser file input only yields bytes, which is why the current mental model collapses into "upload the file." Upload copies bytes into `/api/uploads/` and hands the agent a path — a faithful workaround when the frontend and backend live on different machines, but pure friction when they are the same machine, which is the common case for `octo serve`.
...
- Confirm parks the chosen path until the first message is sent, at which point the session is created inside the project that owns that directory — reusing it when one exists, creating it named after the directory when none does. The picker is offered only on the new-session landing page for that reason: once a session exists, its directory is its project's to change.
