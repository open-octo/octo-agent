...
- **`guard.ts`** (new, TS) and its Go-side counterpart in `render_ui.go`: the security boundary — see "Security design."
...
New file `internal/tools/render_ui.go`.
...
- **Structural caps, enforced identically by the Go guard (`internal/tools/genui/`, used by `render_ui.go`) and the TS guard (`web/src/lib/genui/guard.ts`, used by both the tool-card path as a second, independent pass and the inline-fence path as its only pass):** max tree depth 8, max total node count 200, max string field length 500 (2000 for table cells), max array length 200 for `list`/`items`, 100 for `table.rows`, 50 for `table.columns` (an implementation addition beyond this list — the design left table width unbounded, which the guard code closes as a straightforward extension of the same row/cell caps, not a special case a caller needs to think about), 50 for `select`/`radio` options, 8 for `tabs`. A `button`'s `payload` object is depth- and width-capped by the same rules (depth ≤ 8, ≤ 50 keys/entries per level) but as a **per-node** budget — it is not subtracted from the spec-wide 200-node total, so a spec with many buttons each carrying a near-max payload carries more total JSON than "200 nodes" alone suggests; not a safety gap (this data only ever reaches `JSON.stringify` into an `[octo-ui-action]` reply, never HTML), just a sizing note for whoever tunes these constants later. `progress.value` is clamped to `[0, 100]`. An out-of-whitelist `type` at any position causes that node (and its subtree) to be dropped, not the whole spec — logged as a `console.warn` client-side and as the tool's error text server-side (Slice A only; Slice B's guard runs silently since a dropped inline node has no channel back to the model mid－stream, only the eventual `[octo-ui-action]`/next-turn context if it matters).
...
**Automated coverage**: Go unit tests for `internal/tools/genui` (guard cap/whitelist table-driven cases) and `internal/tools/render_ui_test.go` (tool contract); Vitest unit tests for `fence-split.ts`, `partial-parse.ts`, `guard.ts` (co-located `*.test.ts` files next to their source, this repo's actual convention — there is no separate `web/tests/` directory). No Svelte component-mount test was added: `@testing-library/svelte` is present as a devDependency but `vitest.config.ts` resolves Svelte's server-side build under Vitest, not the client build, so a real `mount()`-based render throws `lifecycle_function_unavailable` without a `resolve.conditions` change to shared test infrastructure — out of scope for this feature. Manual verification substitutes for that gap: an end-to-end browser click-through against a real running instance (an isolated test server, cleaned up afterward) confirmed the tool-card path renders correctly with a real model turn, alongside the logic-level tests above.
