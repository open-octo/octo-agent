...
这套广播只覆盖本进程做的改动。`notifyGroupsChanged` 是 Server 装配的进程内钩子，另一个进程碰不到它——而另一个进程是常态：终端里的 `octo` 会建会话、会调 `EnsureProjectForDir` 归档，第二个 `serve` 或桌面 shell 也能写同一份 `~/.octo`。**这个缺口不限于项目：CLI 建的会话本身也不会推送**，因为 `session_created` 同样是进程内广播，而 `session_list` 是请求-响应式的、不主动推。

数据从不出错——`listSessions` 每次重读目录，`cachedRegistry` 按 mtime+size 重新 stat，刷新页面就是真相。缺的只是"被告知"。

`storeWatch`（`store_watch.go`）补上这一半：每 5 秒采样一次 `~/.octo`，变了就广播已有的那两个事件，前端不用改。指纹刻意只包含"会话增删"和"注册表变更"这两类信号，**不含任何随 transcript 内容变化的东西**——每次广播都会让所有打开的 tab 重新 list 会话（而那要加载每个 transcript），所以一个每轮 turn 都会动的信号会变成整场对话期间的持续开销。这条性质成立是因为 `Session.Save` 走 append，其 rewrite 路径是 `O_TRUNC` 就地截断而不是 temp+rename（rename 会改目录 mtime，那就正好掉进上面那个循环里）。

会话侧同时看**文件数量和目录 mtime**，因为两者互补盲区：数量漏掉"同一间隔内一增一删"，目录 mtime 能抓；目录 mtime 漏掉"同一 mtime 粒度内连续建两个会话"，数量能抓。目录 mtime 还会因为 `<id>.chunks` 归档目录出现而变，那不是新会话——多一次重拉，是更便宜的那个错误方向：多报是幂等的，漏报会让侧栏少一行直到下次变化。

本进程自己的改动也会被采样到并广播一次多余的，这是刻意接受的：前端两个事件都是整体重拉，幂等；而"记账每一次本地写好让 watcher 认出自己"是一大堆容易过期的机械，换来的收益本来就被采样间隔封顶了。
...
- `storeWatch`：另一个进程建的会话与项目都会被广播出去；对已有 transcript 的写入**不**广播（否则整场对话每个间隔都让所有 tab 重载全部 transcript）；空闲采样静默；`watchStop` 关闭后 goroutine 退出。
