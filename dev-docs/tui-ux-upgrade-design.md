...
#149 落地的交互式 TUI（`cmd/octo/tuirepl.go` + `tuirepl_view.go`）是 inline 模式：committed 输出靠 `tea.Println` 进终端 scrollback，底部 live 区（`View()`）= partial 助手行 + `thinking…` + 队列 + `you> ▏` 输入行 + 一行键位提示。它能用，但视觉上很朴素，**关键反差是它比 octo 自己的 plain 路径还简陋**：

| 维度 | TUI 现状（`tuirepl.go`） | plain 路径（`repl.go` `replToolEventHandler`） |
|---|---|---|
| 工具渲染 | 一行 `↳ tool ✓`（`handleEvent` line ~228） | **已用 `internal/tui.RenderEditCard`** 渲染带 chroma 语法高亮的 diff 卡片（`repl.go:654`） |
| 助手文本 | 原始 text delta | 同样原始（两边都无 markdown） |
| 输入 / 状态 | `you> ▏` + 一行 hint | n/a |

也就是说 `internal/tui/diffcard.go`（true-color、行号、±标记、chroma 高亮的精致 diff 卡片）**已存在且已在 plain 路径用**，只是 bubbletea TUI 没接进来。`chroma` 已是依赖。
...
`replToolEventHandler`（plain）已用卡片；本期让 `tuirepl.handleEvent` 走同一套渲染函数，差距即抹平。
...
| 期 | 内容 | 价值 |
|---|---|---|
| **P1** | `internal/tui` theme 包雏形 + 工具富卡片接进 TUI（§4） | 立刻抹平 TUI<plain 差距，收益最大 |
