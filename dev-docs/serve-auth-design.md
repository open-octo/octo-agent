...
| Agent-written HTML (prompt injection can author it) rendering in the panel | Page script calling the app API as the user | **Not defended.** Artifacts and Light Apps render from `/_artifacts/…` and `/_apps/…` on the app's own origin, so the page is same-origin with the UI and holds its credentials. Accepted so they work over any transport; see `same-origin-artifacts-design.md` and SECURITY.md |
