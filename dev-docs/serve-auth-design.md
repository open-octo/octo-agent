...
| Agent-written HTML (prompt injection can author it) rendering in the panel | Script on the artifact origin `<token>.artifacts.localhost` calling the app API | Host routing: that host serves artifact files only, no `/api`/`/ws`/UI. Origin validation: `*.localhost` is not a local name, so the page's `Origin` fails the loopback exemption's CSRF gate (403); the access-key cookie is host-only and never sent to the subdomain. See `artifact-origin-design.md` |
