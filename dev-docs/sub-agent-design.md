...
**lean**(`explore`/`plan`):为压低成本,这两个调研型 agent 默认跑在**父的 lite 模型**上
(`parent.LiteModel` + `LiteSender`,没配 lite 模型则回落到父模型),并用**精简 system**
(`parent.LeanSystem`,丢掉 skills manifest + memory 注入)作为起点。显式传 `model` 时优先用显式的。
...
```go
child := agent.New(sender, model)          // 一般复用父 Sender;lean preset 用 LiteSender
child.System = baseSystem                  // 一般 = parent.System;lean preset = parent.LeanSystem
...
- **共享点**:Sender(一条连接;lean preset 改用父的 LiteSender)、System(同一身份,fork 时连 cache 一起共享;lean preset 用精简 system)、Gate(同一权限——子 agent 不绕过权限)、计费(子 agent token 累加进父 session 总数,`/cost` 报合并数字)。
