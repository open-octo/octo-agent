...
| 名称 | 只读 | leanSystem | liteModel | 用途 |
|------|------|-----------|-----------|------|
| `explore` | 是 | 是 | 否 | 只读调研:定位、理解代码 |
| `plan` | 是 | 是 | 是 | 只读调研后产出计划 |
| `general` | 否 | 否 | 否 | 全工具,端到端处理委派任务 |
| `code-review` | 是 | 否 | 否 | 用 `git diff` 等审查改动 |

lean 拆成两个独立开关:**leanSystem**(`explore`/`plan`)用**精简 system**(`parent.LeanSystem`,
丢掉 skills manifest + memory 注入)作为起点;**liteModel**(仅 `plan`)跑在**父的 lite 模型**上
(`parent.LiteModel` + `LiteSender`,没配 lite 模型则回落到父模型)。`explore` 刻意保留父模型——
它的调研结论直接决定父代理的下一步,只靠 leanSystem 省 token。显式传 `model` 时优先用显式的。
...
```go
child := agent.New(sender, model)          // 一般复用父 Sender;liteModel preset 用 LiteSender
...
- **共享点**:Sender(一条连接;liteModel preset 改用父的 LiteSender)、System(同一身份,fork 时连 cache 一起共享;leanSystem preset 用精简 system)、Gate(同一权限——子 agent 不绕过权限)、计费(子 agent token 累加进父 session 总数,`/cost` 报合并数字)。
