...
- **随处可用**：同一个二进制同时提供交互式 CLI、headless 单发、Web、桌面、IM、编辑器插件、移动端八种入口。
...
- **八种界面**：CLI / TUI、headless 单发、Web UI、桌面应用、IM 桥接、VS Code、Obsidian、移动端——很少有其他 agent 项目能同时覆盖这么多入口。
...
octo（单二进制）
  -> CLI / TUI            终端交互式会话
  -> Headless 单发         octo "提示词" 或管道 stdin：跑完一轮完整 agent 循环后退出，可从 cron、CI 或你自己的程序里调用
...
**稳定版（1.0）** 已覆盖 CLI（交互式 TUI 与 headless 单发）、Web UI、桌面端、IM 桥接、[VS Code](https://github.com/open-octo/octo-vscode) / [Obsidian](https://github.com/open-octo/octo-obsidian) 插件；第八个界面——移动端 App（iOS + Android）——已实现，目前是**开发者预览**：现在即可从源码自建、配合自托管 relay 使用（见 [`mobile/`](mobile/)），托管 relay 与应用商店版本在路上。
...
octo 已发布 **1.0 稳定版**，CLI（交互式与 headless 单发）、Web UI、桌面端、IM 桥接、编辑器插件均可放心使用；移动端处于开发者预览。哪些接口可以放心依赖见 [COMPATIBILITY.md](COMPATIBILITY.md)；安全边界见 [SECURITY.md](SECURITY.md)。
