...
> 一个 **MIT 开源、单 Go 二进制、零运行时**的 AI Agent，把大家平时要装两套工具才能凑齐的能力揽到了一起：
> **coding agent 能力对标 Claude Code**，**个人助手能力比 OpenClaw 更轻量、更稳定**——skills、CLI / 网页 /
> 桌面 / 手机 IM / VS Code / Obsidian / Go SDK（移动端 App 即将上线）、浏览器操作、OS 级沙箱全都有，且**完全开源、自包含、不绑任何厂商**，DeepSeek / Kimi / Anthropic /
> OpenAI 任意模型都能接，服务和数据都留在你自己机器上。复用你已有的 Claude Code 技能。一个二进制
> 同时接管你的编码和日常自动化，不必装两套工具。
...
## 为什么用 octo（对比 OpenClaw）

在个人助手这条线上，[OpenClaw](https://github.com/openclaw/openclaw) 是最接近的同类 —— 自托管、MIT
开源、消息驱动，能在你已经在用的聊天 App 里找到你。octo 覆盖同一块地盘，但它是单个静态二进制而不是带一整棵
依赖树的 Node.js 应用，并且自带完整的 coding agent 核心（和上面 Claude Code 对比里同一套工具 / 权限 /
沙箱栈），不只是一个助手。

|  | **octo-agent** | OpenClaw |
|---|---|---|
| 运行时 | **单个静态 Go 二进制 —— 无需 Node 运行时，无需 `npm install`，没有需要打补丁 / 重新编译的依赖树** | Node.js（推荐 Node 24）+ 它的 npm 依赖树 |
| 编码能力 | **内置完整的 agentic 编码循环** —— 文件读写改、terminal、权限门控、OS 沙箱 | 以助手 / 自动化为主，不是围绕 coding agent 核心构建 |
| IM 渠道 | 微信 iLink / 飞书 / 钉钉 / 企微 / Discord / Telegram | WhatsApp / Telegram / Discord / Slack / Signal / Matrix / 微信 / QQ 等 15+ 种 |
| 授权 / 部署 | MIT，自托管 | MIT，自托管 |

<sub>OpenClaw 信息据其公开仓库 / 文档（2026-07）。两者都是自托管、MIT 开源、消息驱动的个人助手 —— octo
这里的优势是跑成单个静态二进制（没有需要打补丁或重新编译的东西）且自带 coding agent 核心，而不是渠道数量
的比拼；OpenClaw 目前开箱支持的 IM 渠道更多。</sub>

**一句话**：编码能力上，octo 想对标 Claude Code；日常助手场景上，它想做比 OpenClaw 更轻量、更稳定的替代
方案 —— 一个二进制顶替两套工具。

## 状态

> **稳定版（1.0）。** 规划了八个界面 —— 正好对上章鱼的八条腕 —— 其中七个已上线：CLI（终端里是交互式 TUI，其余场景是 headless 的 agentic 单发）、本地 Web 服务（`octo serve`）、原生桌面应用（`cmd/octo-desktop` —— Wails 窗口内嵌同一套 UI + 系统托盘，进程内直接跑 hub，无需管理浏览器或后台服务）、IM 桥接（随 `octo serve` 运行；微信 iLink、飞书、钉钉、企微、Discord、Telegram）、VS Code 插件（[`open-octo/octo-vscode`](https://github.com/open-octo/octo-vscode)）、Obsidian 插件（[`open-octo/octo-obsidian`](https://github.com/open-octo/octo-obsidian)）、Go SDK（[`pkg/octoagent`](pkg/octoagent) —— 把同一套 agent 循环嵌进你自己的 Go 程序，无需 internal 依赖）—— 其中 VS Code 与 Obsidian 客户端通过与 Web UI 相同的 HTTP/WebSocket API 连接 `octo serve`。第八个界面移动端 App 即将上线。在 agent 循环之上还有 skills、MCP 客户端、操作系统级沙箱、持久化记忆、子代理、后台工作流，以及用于自主多步目标的任务图。
>
...
下载 `Octo-x86_64.AppImage`，`chmod +x` 后运行。它需要主机上有 GTK4 +
WebKitGTK 6.0（多数较新的桌面环境都自带；缺了的话应用会提示你装什么）。
...
- **双击安装器（桌面应用）。** 从[最新 release](https://github.com/open-octo/octo-agent/releases/latest)
  下载 `octo-setup.pkg`（一个通用包同时覆盖 Apple Silicon 和 Intel），双击
  运行。Installer.app 只提供"仅为我安装"这一个选项 —— 不需要管理员密码。它
  把 **Octo 桌面应用**装到 `~/Applications/Octo.app`，并把随附的 `octo` CLI
  加进你的 `PATH`（追加到 `~/.zprofile` / `~/.bash_profile` / `~/.profile`），
  然后打开这个应用 —— 一个原生窗口，无需管理浏览器或后台服务。想用终端的话，
  打开一个**新**终端窗口跑 `octo` 即可。应用尚未经过 Apple 公证，Gatekeeper
  会提示"来自身份不明的开发者" —— 右键（或按住 Control 点击）`Octo.app` →
  **打开**，或者去**系统设置 → 隐私与安全性 → 仍要打开**。`.pkg` 没有 App
  Store 那种自带卸载器，运行 `~/.octo/uninstall.sh` 即可移除（你的 `~/.octo`
  数据会保留）。

**Windows —— 两种安装方式：**

- **安装脚本（PowerShell）。** 对应 `curl … | sh` 一行命令的 Windows 版 ——
  自动识别架构、校验 SHA-256、按用户级安装 `octo.exe` 并加进 `PATH`：
...
- **双击安装器（桌面应用）。** 从[最新 release](https://github.com/open-octo/octo-agent/releases/latest)
  下载 `octo-setup.exe`，双击运行。它按用户级安装（不弹管理员提示）**Octo
  桌面应用**加 `octo` CLI，把 CLI 加进你的 `PATH`，并添加一个启动该应用的
  开始菜单项。安装完成后会打开这个应用 —— 一个原生窗口，没有需要在后台管理
  的东西。想用终端的话，打开一个**新**终端窗口跑 `octo` 即可。安装器暂未做
  代码签名，首次运行时 Windows SmartScreen 会提示"Windows 已保护你的电脑"
  —— 点**更多信息 → 仍要运行**即可。卸载走"添加或删除程序"，和普通软件一样。
...
octo 的 Windows 安装器（`octo-setup.exe`）通过 [SignPath Foundation](https://signpath.org/)
的免费开源计划签名，证书由其签发；签名私钥由 SignPath 保管，从不离开其基础
设施。macOS 安装器（`octo-setup.pkg`）目前尚未签名或公证 —— 本项目背后没有
Apple Developer Program 会员资格 —— 所以在证书情况改变之前，每个 release 在
Gatekeeper 那里都会出现"身份不明的开发者"提示。

- **签名对象：** 每个 GitHub release 附带的 `octo-setup.exe` 安装器。release
  压缩包本身通过随附的 `checksums.txt` 做完整性校验。
- **方式：** 安装器在 CI（`.github/workflows/release.yml`）中构建，并作为发布
  流程的一部分提交给 SignPath 签名。每次签名请求都需项目维护者批准后才签。
- **谁能批准：** 仓库维护者。任何拥有签名请求批准权或仓库写权限的人，都在
  GitHub 与 SignPath 两侧启用多因子认证。
- **验证：** 右键安装器 → *属性 → 数字签名* 查看发布者。

签名正在通过 SignPath Foundation 逐步落地；在证书就绪之前，发布的安装器可能
**未签名**，此时 Windows SmartScreen 会提示"Windows 已保护你的电脑" —— 点
**更多信息 → 仍要运行**即可。
...
| 桌面应用 | 完成 | `cmd/octo-desktop` —— 原生 Wails GUI，进程内运行 hub（同一套 Web UI 装进窗口 + 系统托盘，无需管理浏览器/后台服务）；macOS `.pkg`、Windows `.exe`、Linux AppImage |
| IM 桥接 | 完成 | 随 `octo serve` 运行 —— 微信 iLink/飞书/钉钉/企微/Discord/Telegram 适配器（web 扫码登录、按用户隔离 session、斜杠命令） |
| Go SDK | 完成 | `pkg/octoagent` —— 公开 API，把同一套 agent 循环（turn、工具、权限门、hooks）嵌进你自己的 Go 程序，无需 internal 依赖 |
