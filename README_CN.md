...
> **Pre-1.0。** 三种界面均已上线：CLI/REPL、本地 Web 服务（`octo serve`）、IM 桥接（`octo channel`，微信 iLink）。在 agent 循环之上还有 skills、MCP 客户端、操作系统级沙箱、持久化记忆、子代理，以及用于自主多步目标的任务图。
...
# 自主多步目标：拆成子任务 DAG 后执行
octo goal start "给 octo config show 加一个 --json 标志"
octo goal status <id>
```
...
| 任务图 | 完成 | `octo goal` —— 把目标拆成子任务 DAG，经子代理执行，崩溃后可 resume |
...
```
cmd/octo/          CLI 入口（chat / REPL / serve / channel / goal / mcp / 斜杠命令）
   ↓
...
internal/taskgraph/  octo goal —— 子任务 DAG 规划器 + 调度器
```
