...
- **数据留在你的机器上**：自托管、零遥测——没有埋点、没有崩溃上报、没有用量回传。不需要你开口就会联网的只有两处：向 GitHub 查有没有新版本（`update_check: false` 可关），以及 Web UI 的图标走 Iconify CDN。两者都不携带你的数据。除此之外，就只有你自己配置的模型 API 调用。
...
- **零遥测**：不收集 IP、机型、模型选择、使用行为，没有任何遥测埋点。octo 唯一会向网络打听自己的事情是「有没有新版本」（不带任何数据，只是一次查询），改一个配置就能彻底静默。另外 Web UI 的图标资源走 Iconify CDN，详见上面的「数据留在你的机器上」。
