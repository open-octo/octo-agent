...
- **数据留在你的机器上**：自托管、零遥测。除了你自己配置的模型 API 调用，octo 唯一自己发起的请求——不是工具调用，也不是你交代的任务——是向 GitHub 查一下有没有新版本，一个开关（`update_check: false`）就能关掉——关掉之后，离开这台机器的就只剩你自己的模型调用。
...
- **零遥测**：不收集 IP、机型、模型选择、使用行为，没有任何遥测埋点。唯一一处联网回家是查询有没有新版本（不带任何数据，只是一次查询），改一个配置就能彻底静默。
