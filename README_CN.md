...
> 桌面 / 手机 IM / VS Code / Obsidian、浏览器操作、OS 级沙箱全都有，且**完全开源、自包含、不绑任何厂商**，DeepSeek / Kimi / Anthropic /
...
> **稳定版（1.0）。** 六种界面均已上线：CLI（终端里是交互式 TUI，其余场景是 headless 的 agentic 单发）、本地 Web 服务（`octo serve`）、原生桌面应用（`cmd/octo-desktop` —— Wails 窗口内嵌同一套 UI + 系统托盘，进程内直接跑 hub，无需管理浏览器或后台服务）、IM 桥接（随 `octo serve` 运行；微信 iLink、飞书、钉钉、企微、Discord、Telegram）、VS Code 插件（[`open-octo/octo-vscode`](https://github.com/open-octo/octo-vscode)）、Obsidian 插件（[`open-octo/octo-obsidian`](https://github.com/open-octo/octo-obsidian)）—— 其中 VS Code 与 Obsidian 客户端通过与 Web UI 相同的 HTTP/WebSocket API 连接 `octo serve`。在 agent 循环之上还有 skills、MCP 客户端、操作系统级沙箱、持久化记忆、子代理、后台工作流，以及用于自主多步目标的任务图。
>
