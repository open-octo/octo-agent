...
七篇下来，从装机到一个真正每周在跑的自动周报，再到用脚本把几个 agent 拼成并行流水线——中间用到的东西始终不多：一次安装、一句自然语言、一份 JSON、一个斜杠命令、一个 cron 表达式、一段十几行的 Ruby 脚本。复杂的部分（技能怎么写、协议怎么接、调度怎么管、并发怎么控）始终留在背后，留给你的只是把需求说清楚。
