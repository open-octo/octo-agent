...
description: "从五层单向依赖讲起，拆解 octo-agent 核心机制的设计与取舍：上下文压缩、prompt 缓存、loop、workflow 编排、浏览器录制回放、权限管控、回收站、MCP 工具搜索、自带电池式入门引导，以及把它们串起来的内置 tool 契约。"
pubDate: 2026-07-08
...
### show_artifact：文件路径不是 UI

大多数产物从 `write_file` / `edit_file` 进入用户视线（Web 的 Artifacts 面板会自动呈现它们）。但是由其它方式产出的文件——某步构建、`cat > x.html` 这个 heredoc、下载的图片——不搭那条车。`show_artifact` 架这个 gap：它对一个预览类型白名单校验路径（`artifactContentTypes`：`.html/.md/.png/.jpg/.gif/.svg/.webp`），确认是个真文件，然后在结果信封里发一个 `UI` 载荷 `{"type": "artifact", "path": ..., "size_bytes": N}`。

两道关：路径必须通过 content-type 白名单（避免渲染器收到不能展示的 binary），产物端点拿 session transcript 里对应 `tool_use` 块当作授权凭证来服这个文件——一个用户的产物不能被另一个 session 预览。HTML 产物在**沙箱 iframe** 里渲染，默认不跑脚本；跑脚本需要用户按文件单独授权。这和别处是一样的"推迟 + 卡关"模式：默认简单且安全，强力路径必须做一次自觉的选择。
...
| Artifact 预览卡关 | `internal/tools/artifact.go`、`internal/server/artifact.go` |
