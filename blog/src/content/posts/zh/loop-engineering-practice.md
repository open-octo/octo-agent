...
octo-agent 是什么、六大构建块、L1/L2/L3 档位的完整定义，[系列第一篇](/blog/posts/loop-engineering-intro/)已经讲过，这里不再重复——直接进入实战。
...
这也是 octo 与一般 coding agent 的分野：它追求的是「人类在关键决策点介入，机器处理所有重复执行」，而非「完全替代人类」。
...
看到下表三个循环全是 L3，先别觉得和前面"第一版永远 report-only"的告诫矛盾——这是**已经走完 L1 → L2 → L3 升级流程之后的稳态**：每个循环都先以只读模式跑过多轮、分类质量和真实产出验证通过，才逐档提上来的。
...
Loop Engineering 要真正落地，每个环节都得落到可运行、可回滚、可观察的 workflow 上。这次实践验证了：
