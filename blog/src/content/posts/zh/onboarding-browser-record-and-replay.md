...
每一步会记录：动作类型（`click`/`type`/`select`/`upload`/`navigate` 这些）、一个锚定在最近的、带稳定 `id` 的祖先元素上的选择器（而不是那种一变页面结构就失效的、按位置算出来的选择器链）、这个元素当时的可见文本，以及所在的 URL——外加目标元素的一份冗余**指纹**：用不同策略生成的备选选择器、它的 `role` 属性、旁边最近的标签式文字。页面的 CSS class 整体翻新之后还能重新找到元素，靠的就是这份指纹（下面回放一节细说）。

录制器还会留意你的操作**之间**发生了什么。一次点击触发了网络请求，就自动插一个"等网络"的步骤；弹出了 modal 或日期选择器，就插一个"等元素"的步骤——回放不会抢在页面就绪之前执行下一步。一次点击触发了文件下载，这个点击会升级成 `download` 步骤并绑定一个 `file[]` 输出。那些一眼就能确证的手滑——同一个字段反复重打、点过去又点回来——会在任何模型看到录制之前被确定性地压缩掉。你喊停之后，`record_stop` 会回一份带编号的执行计划（每一步做什么、检查什么），你确认过，这段录制才算定稿。
...
- 带指纹的步骤靠**评分**重新识别目标：候选元素（来自原始选择器、备选选择器、按文本/role 扫描）逐个和录制时的文本、role、标签名、邻居文字比对打分。CSS class 哈希全换的页面照样定位得到；而旧的位置选择器如今指向了**错误**元素时，这一步会直接拒绝、失败进自愈，绝不静默点错。（指纹功能之前录的旧录制保持原有行为：先找原始选择器，再找带着录制时那段可见文字的元素。）
- 自动插入的 `wait` 步骤会等页面就绪——网络空闲，或指定元素出现——再执行下一步。
- `download` 步骤点击触发器、等下载完成，把落盘文件的路径绑定到它的输出上。
...
如果某一步的选择器和 `hint` 都找不到东西了——并且只有在配置了专门用于这个用途的模型时——octo 会对页面当前可交互的元素取一份纯文本摘要（选择器 + 可见文本，不截图），连同预期动作、预期的文字标签、失效的旧选择器，以及（步骤带指纹时）录制下来的 role / 标签名 / 邻居文字一起发给模型，让它只回一个修正后的 CSS 选择器。修正会被重试一次，成功的话会**直接写回这段录制的 YAML 文件**——所以一次自愈是持久生效的，不是临时补丁，以后每次回放都不用再修一遍。
